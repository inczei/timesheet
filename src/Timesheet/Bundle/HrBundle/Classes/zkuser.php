...
//        $command_string = str_pad(unichr( $uid ), 2, chr(0)).chr($role).str_pad($password, 8, chr(0)).str_pad($name, 28, chr(0)).str_pad(chr(1), 9, chr(0)).str_pad($userid, 8, chr(0)).str_repeat(chr(0),16);
    ...
    function zkdeleteuser($self, $uid) {
    	$command = CMD_DELETE_USER;
    	$command_string = str_pad(chr( $uid ), 2, chr(0));
    	$chksum = 0;
    	$session_id = $self->session_id;
